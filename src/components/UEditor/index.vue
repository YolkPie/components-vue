...
export default {
  ...
  data () {
    return {
      ...
      editorConfig: {
          ...
          initialFrameHeight: this.height, // 初始容器高度
          initialFrameWidth: this.width, // 初始容器宽度
  ...
  created () {
    window.UEDITOR_HOME_URL = this.editorConfig.UEDITOR_HOME_URL
  ...
  methods: {
    ...
    insertScriptTag () {
      ...
      // let langScriptTag = document.getElementById('configScriptTag');

      // 如果这个tag不存在，则生成相关代码tag以加载代码
      if (editorScriptTag === null) {
        configScriptTag = document.createElement('script');
        configScriptTag.type = 'text/javascript'; 
        configScriptTag.src = this.editorConfig.UEDITOR_HOME_URL + 'ueditor.config.js'; 
        configScriptTag.id = 'configScriptTag';

        // langScriptTag = document.createElement('script');
        // langScriptTag.type = 'text/javascript'; langScriptTag.src = this.ueditorPath + 'lang/zh-cn/zh-cn.js'; langScriptTag.id = 'langScriptTag';

        editorScriptTag = document.createElement('script');
        editorScriptTag.type = 'text/javascript'; 
        editorScriptTag.src = this.editorConfig.UEDITOR_HOME_URL + 'ueditor.all.js'; 
    ...
    initEditor () {
      ...
      if (this.scriptTagStatus === 2 && this.instance === null) {
        ...
        this.$nextTick(() => {
          this.instance = window.UE.getEditor(this.randomId, this.editorConfig);
